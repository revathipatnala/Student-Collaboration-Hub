...
export const sendSignupOTP = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Only allow: something.23.two or three letters@anits.edu.in
    const emailRegex = /^[a-zA-Z0-9]+\.[0-9]{2}\.[a-zA-Z]{2,3}@anits\.edu\.in$/;
    if (!emailRegex.test(email)) {
      res.status(400).json({ error: 'Please enter a valid institutional email address (e.g., name.23.xx@anits.edu.in or name.23.xxx@anits.edu.in)' });
      return;
    }
