...
export const sendSignupOTP = async (req, res) => {
    try {
        ...
        // Only allow: something.two digit number.three letters@anits.edu.in
        const emailRegex = /^[a-zA-Z0-9]+\.[0-9]{2}\.[a-zA-Z]{3}@anits\.edu\.in$/;
        if (!emailRegex.test(email)) {
            res.status(400).json({ error: 'Please enter a valid institutional email address (e.g., name.23.abc@anits.edu.in)' });
            return;
        }
